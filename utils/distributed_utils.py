...
rank_zero_warn = rank_zero_only(warn)
...
def all_gather(data):
    ...
    """
    to_device = torch.device(f"cuda:{get_local_rank()}")
...
def all_gather_cpu(result_part, tmpdir=None, collect_by_master=True):
    ...
    if tmpdir is None:
        tmpdir = "./tmp"
    if rank == 0:
        os.makdirs(tmpdir, exists_ok = True)
    synchronize()
    # dump the part result to the dir
    pickle.dumps(result_part, os.path.join(tmpdir, f"part_{rank}.pkl"))
    synchronize()
    # collect all parts
    if collect_by_master and rank != 0:
        ...
        for i in range(world_size):
            ...
            results.append(pickle.load(part_file))
