...
def _init_linear_normal(m: nn.Linear, std: float = 0.02):
    """Gaussian init for Linear: weight ~ N(0, std^2), bias = 0."""
    nn.init.normal_(m.weight, std=std)
    if m.bias is not None:
        nn.init.zeros_(m.bias)

def _init_linear_xavier(m: nn.Linear, gain: float = 1.0):
    """Xavier-uniform init for Linear with given gain, bias = 0."""
...
def init_attention(
    ...
    1) LocalFrameAttention-style   : attn.qkv_proj / attn.out_proj
    2) diffusers-style                  : attn.to_q / attn.to_k / attn.to_v / attn.to_out[0]

    Args:
        attn: Attention module to initialize.
        mode:
            - "safe": Xavier-uniform for all Linear weights. Good default.
            - "deep": Xavier-uniform + residual scaling on output projection
                      by 1/sqrt(2 * num_layers) for stability in deep nets.
            - "normal": Gaussian N(0, 0.02) for all Linear weights, bias=0.
                        Matches many diffusers checkpoints.
        num_layers: Total number of transformer layers (for residual scaling).
                    Required when mode="deep".
        std: Std for Gaussian init when mode="normal".
    """
    # Case 1: LocalFrameAttention flavor
    if hasattr(attn, "qkv_proj") and hasattr(attn, "out_proj"):
        if mode in ("safe", "deep"):
            _init_linear_xavier(attn.qkv_proj)
            _init_linear_xavier(attn.out_proj)
        elif mode == "normal":
            _init_linear_normal(attn.qkv_proj, std=std)
            _init_linear_normal(attn.out_proj, std=std)
        else:
            raise ValueError(f"Unknown mode: {mode}")

        # Residual scaling on the output projection for deeper models
        if mode == "deep" and num_layers is not None and num_layers > 0:
            with torch.no_grad():
                attn.out_proj.weight.mul_(1.0 / math.sqrt(2.0 * num_layers))
        return

    # Case 2: diffusers flavor
    if all(hasattr(attn, name) for name in ("to_q", "to_k", "to_v")):
        qkv_linears = [attn.to_q, attn.to_k, attn.to_v]
        # to_out may be a nn.ModuleList([Linear, Dropout]) or a bare Linear
        out_linear = None
        if hasattr(attn, "to_out"):
            out_linear = attn.to_out[0] if isinstance(attn.to_out, nn.ModuleList) else attn.to_out

        if mode in ("safe", "deep"):
            for m in qkv_linears:
                _init_linear_xavier(m)
            if out_linear is not None:
                _init_linear_xavier(out_linear)
        elif mode == "normal":
            for m in qkv_linears:
                _init_linear_normal(m, std=std)
            if out_linear is not None:
                _init_linear_normal(out_linear, std=std)
        else:
            raise ValueError(f"Unknown mode: {mode}")

        if mode == "deep" and num_layers is not None and num_layers > 0 and out_linear is not None:
            with torch.no_grad():
                out_linear.weight.mul_(1.0 / math.sqrt(2.0 * num_layers))
        return

    # Fallback: unsupported structure
    raise TypeError(f"Unsupported attention module type: {type(attn)}")