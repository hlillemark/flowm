...
class HistoryGuidanceManager:
    ...
    def __enter__(self):
        ...
        seq_len, hist_len, gen_len = ( # seq_len is the window size of the model
            len(reduced_mask),
        ...
        # NOTE: this is where we convert (0, 1) to [0, 1000)
    ...
    def prepare(
        ...
        if not replacement_only:
            # NOTE: only cope with history tokens, the tokens to generate is managed by that sheduling matrix
...
class HistoryGuidance:
    ...
    @classmethod
    def conditional(
        cls, timesteps: int = 1000, visualize: bool = True, **kwargs
    ) -> "HistoryGuidance":
        """
    ...
    def stabilized_conditional(
        cls,
        stabilization_level: float = 0.02,
        timesteps: int = 1000,
        visualize: bool = True,
        **kwargs,
    ) -> "HistoryGuidance":
