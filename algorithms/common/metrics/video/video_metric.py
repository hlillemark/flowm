from typing import List, Optional, Set, Tuple, Iterable, Union
import torch
...
class VideoMetric(nn.Module):
        ...
        for metric_type in metric_types:
            match metric_type:
                ...
                case VideoMetricType.VBENCH | VideoMetricType.REAL_VBENCH:
                    module = VBench(registry=registry, resolution=resolution)
    ...
    def forward(
        ...
        if mask is not None:
            ...
            mask_split = mask.chunk(self.split_batch_size, dim=0)
        else:
            mask_split = None
    ...
    def _update(
        ...
        for metric_type, module in self._filtered_items(self.FRAME_WISE_METRICS):
            if metric_type == VideoMetricType.MASKED_MSE:
                # Masked MSE is computed only for non-context frames
                module.update(preds, target, mask=mask)
            else:
                module.update(preds, target)
