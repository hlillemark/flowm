...
class VBench(Metric):
    ...
    def __init__(
        ...
        resolution: Union[int, Tuple[int, int]] = (360, 640),
        **kwargs: Any,
    ) -> None:
        """
        Args:
        ...
        self.dimensions = torch.nn.ModuleDict(
            {
                ...
                VBenchDimensionType.BACKGROUND_CONSISTENCY: BackgroundConsistency( # Use CLIP
                    registry
                ),
                VBenchDimensionType.TEMPORAL_FLICKERING: TemporalFlickering(registry),
                VBenchDimensionType.AESTHETIC_QUALITY: AestheticQuality(registry), # Use CLIP
    ...
    def compute(self) -> Dict[str, Tensor]:
        ...
        output_scores.update(
            {
                f"{dimension.value}_normalized": normalized_score
                for dimension, normalized_score in normalized_scores.items()
            }
        )
        output_scores["final"] = final_score
