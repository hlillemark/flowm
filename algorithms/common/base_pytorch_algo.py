...
from algorithms.common.metrics.video import VideoMetric, SharedVideoMetricModelRegistry, filter_excluded_video_metrics
from utils.distributed_utils import reduce_dict
    ...
    def on_validation_epoch_end(self, namespace="validation") -> None:
        ...
        if self.trainer.state.fn != "FIT":
            for task in self.tasks:
                try:
                    metrics_dict = self._metrics(task).log(task)

                    # split into what to log vs what to handle separately
                    to_log, to_log_manual = filter_excluded_video_metrics(metrics_dict)

                    data = [[v.compute() for v in to_log.values()]] # [[v1, v2, v3]], scalar values

                    _ = [v.reset() for v in to_log.values()]
                    
                    # Ensure all ranks reach this point before performing collective operations
                    # This prevents NCCL timeout when some ranks finish inference earlier than others
                    import torch.distributed as dist
                    if dist.is_available() and dist.is_initialized():
                        dist.barrier()
                    
                    # sync the data across all ranks
                    to_log_dict = {list(to_log.keys())[i]: data[0][i] for i in range(len(list(to_log.keys())))}
                    to_log_dict = reduce_dict(to_log_dict)


                    if is_rank_zero:
                        columns = ["name"] + list(to_log_dict.keys())
                        data = [[self.logger.experiment.name] + list(to_log_dict.values())]

                        table = wandb.Table(columns=columns, data=data)
                        wandb.log({f"validation_metrics_table/{task}": table, "step": self.global_step}, commit=False)

                        # normal Lightning log (wandb)
                        # don't need to pass into logger here, it's handled automatically inside
                        self.log_dict(
                            to_log_dict,
                            on_step=False,
                            on_epoch=True,
                            prog_bar=False,
                            sync_dist=False, # already reduced by reduce_dict
                        )
